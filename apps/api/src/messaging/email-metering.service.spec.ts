...
describe("EmailMeteringService", () => {
  beforeEach(() => {
    ...
    mockReturning.mockResolvedValue([{ included: 300, used: 0 }]);
  });

  it("creates default monthly credits with free cap", async () => {
    const service = new EmailMeteringService();
    queuedWhereResults = [{ limit: mockLimit }];
    mockLimit.mockResolvedValueOnce([]);

    const out = await service.getOrCreateCredits("org-1", "2026-05");
    expect(out).toMatchObject({ included: 300, used: 0, remaining: 300, total: 300 });
  });

  it("blocks when cap is reached", async () => {
    const service = new EmailMeteringService();
    queuedWhereResults = [{ limit: mockLimit }];
    mockLimit.mockResolvedValueOnce([{ included: 300, used: 300 }]);
