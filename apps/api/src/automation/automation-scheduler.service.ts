...
export class AutomationSchedulerService {
  ...
  async runAppointmentReminders() {
    ...
    await this.jobRuns.record("appointment_reminders", async () => {
      const db = getDb();
      const now = new Date();
      ...
      const tasks = [
        ...appointments24h.map((a) =>
          this.automationSend.sendAppointmentReminder24h(
            a.organizationId,
            a.businessId,
            a.id,
          ),
        ),
        ...appointments72h.map((a) =>
          this.automationSend.sendAppointmentReminder72h(
            a.organizationId,
            a.businessId,
            a.id,
          ),
  ...
  async runInactivityWinback() {
    ...
    await this.jobRuns.record("inactivity_winback", async () => {
      const db = getDb();
      ...
      for (const s of settingsRows) {
        ...
        tasks.push(
          ...inactiveCustomers.map((c) =>
            this.automationSend.sendInactivityWinback(
              s.organizationId,
              s.businessId,
              c.id,
            ),
