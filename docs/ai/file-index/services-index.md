...
| `apps/api/src/billing/billing.service.ts` | subscription/business billing logic | `BillingService` | paymongo + DB | billing controllers | `starter/growth/pro` pricing, checkout, state changes | High |
...
| `apps/api/src/common/*.service.ts` | shared policy and feature flags | service classes | env + helpers | many API modules | freemium gating, org billing state, plan capacity policies | High |
