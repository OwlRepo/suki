...
| `apps/api/src/billing/billing.service.ts` | subscription/business billing logic | `BillingService` | paymongo + DB | billing controllers | pricing, checkout, state changes | High |
...
| `apps/api/src/licensing/*.service.ts` | license and OTA logic | service classes | signing keys/artifacts | licensing controllers | entitlement and update validation | High |
...
| `apps/api/src/common/*.service.ts` | shared policy and feature flags | service classes | env + helpers | many API modules | cross-cutting business policies | High |
