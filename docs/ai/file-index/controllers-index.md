...
| `apps/api/src/billing/billing.controller.ts` | checkout and billing actions | `BillingController` | billing/paymongo services | settings/billing UI | subscription and add-on flows | High |
...
| `apps/api/src/crm/*.controller.ts` | CRM resources | controller classes | CRM services | CRM pages | CRUD and pipeline management | High |
...
| `apps/api/src/licensing/*.controller.ts` | licensing + OTA endpoints | controller classes | licensing services | licensing clients/admin | license verify/update checks | High |
