...
| `apps/web/src/app/sign-in/page.tsx` | Custom first-party sign-in route | Uses `auth-client` OTP/password APIs and router redirect | Email OTP sign-in with password fallback after retry threshold |
| `apps/web/src/app/sign-up/page.tsx` | Custom first-party sign-up route | Uses `auth-client` OTP APIs and router redirect | Email OTP account creation and immediate dashboard redirect |
