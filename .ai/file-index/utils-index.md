...
| `apps/web/src/lib/auth-client.ts` | First-party auth API client helpers | Consumed by sign-in/sign-up pages/session hook and uses `api-base` | Cookie-credentialed password sign-in preserving redirect metadata plus sign-up OTP/password calls through production `/api/auth/*` proxy routing |
