...
- Tyvera Assistant Markdown is assistant-only: user messages remain plain text, raw HTML is skipped, and Markdown links are clickable only for fixed internal Tyvera routes. Stream display timing is a client-side progressive buffer with reduced-motion bypass and near-bottom following that must not override intentional upward scrolling.
...
- Assistant model execution belongs to `apps/api/src/help/*` and must pass through `AiExecutionService` guardrails. Native streaming and tools remain independently disabled by default through centralized feature-flag methods, with the deterministic concise plain-text assistant flow retained as the rollback path. Native response normalization may preserve restricted Markdown only after jargon replacement, raw-HTML stripping, and length capping; optional draft-only notices must be derived from executed server tool history rather than model-authored fields. The existing SSE event contract, terminal-error behavior, and transport/protocol fallback semantics remain unchanged.
