...
- Operations monitoring uses `automation_job_runs` for deterministic scheduler execution counts, `provider_health_snapshots` for Semaphore/Resend health history, and `operations_alerts` for open/acknowledged/resolved provider, delivery, OTP, and missing-run alerts.
- Monitoring summaries compare `message_events.delivery_status` and the nullable fallback `message_events.status` with explicit predicates because the columns are separate PostgreSQL enum types.
...
- Semaphore reconciliation and automation reminder schedulers record run metadata in `automation_job_runs` before exposing history to platform-admin operations pages.
