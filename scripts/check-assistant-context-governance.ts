...
function main() {
  ...
  const aiIndexFiles = changed.filter((file) => file === "docs/ai/file-index/repository-map.md");
  const aiArchitectureManifestFiles = changed.filter(
    (file) => file === "docs/ai/architecture-manifest.md",
  );
